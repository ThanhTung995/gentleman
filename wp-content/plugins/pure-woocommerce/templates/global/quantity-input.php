...
if ( $max_value && $min_value === $max_value ) {
	...
	?>
	<div class="quantity">
		<span class="dec"><i class="far fa-caret-down"></i></span>
		<input type="number" class="input-text qty text color-heading" step="<?php echo esc_attr( $step ); ?>" min="<?php echo esc_attr( $min_value ); ?>" max="<?php echo esc_attr( 0 < $max_value ? $max_value : '' ); ?>" name="<?php echo esc_attr( $input_name ); ?>" value="<?php echo esc_attr( $input_value ); ?>" title="<?php echo esc_attr_x( 'Qty', 'Product quantity input tooltip', 'woocommerce' ) ?>" size="4" pattern="<?php echo esc_attr( $pattern ); ?>" inputmode="<?php echo esc_attr( $inputmode ); ?>" />
		<span class="inc"><i class="far fa-caret-up"></i></span>
	</div>
	<?php
}
